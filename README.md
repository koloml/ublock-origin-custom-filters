# uBlock Origin Custom Filters

Small set of custom filters for annoying parts of the sites I'm often using. This list is only used & tested for 
uBlock Origin extension.

## How to add

You can subscribe to specific filters by clicking the links below. You can subscribe only to specific parts of the list
in case you don't want some of these filters to be applied. Each parent filter will automatically include all the child
filters in the list.

Unfortunately, there is no way to make these filters toggleable in the uBlock Origin interface.

### Lists

- [All Filters](https://subscribe.adblockplus.org/?location=https://github.com/koloml/ublock-origin-custom-filters/raw/refs/heads/master/filters.txt&title=Kolo's+Custom+Filters) — This will include all filters created in the repository at once.
  - [YouTube](https://subscribe.adblockplus.org/?location=https://github.com/koloml/ublock-origin-custom-filters/raw/refs/heads/master/sites/youtube.txt&title=Kolo's+Custom+Filters+-+YouTube) — Hides parts of the YouTube I didn't like.
    - [Shorts](https://subscribe.adblockplus.org/?location=https://github.com/koloml/ublock-origin-custom-filters/raw/refs/heads/master/sites/youtube/shorts.txt&title=Kolo's+Custom+Filters+-+YouTube+Shorts) — Hide YouTube Shorts from sidebar and from the recommendations.
    - [Membership Videos](https://subscribe.adblockplus.org/?location=https://github.com/koloml/ublock-origin-custom-filters/raw/refs/heads/master/sites/youtube/membership.txt&title=Kolo's+Custom+Filters+-+YouTube+Memebrship) — Hide members-only videos from the sidebar.
    - [AI Features](https://subscribe.adblockplus.org/?location=https://github.com/koloml/ublock-origin-custom-filters/raw/refs/heads/master/sites/youtube/gemini.txt&title=Kolo's+Custom+Filters+-+YouTube+AI+Features) — Hides video summary and other sections I might find related to AI.
    - [Search](https://subscribe.adblockplus.org/?location=https://github.com/koloml/ublock-origin-custom-filters/raw/refs/heads/master/sites/youtube/search.txt&title=Kolo's+Custom+Filters+-+YouTube+Search) — Hides unrelated data from the search results.